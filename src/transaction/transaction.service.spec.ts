...
describe('TransactionService', () => {
  ...
  describe('findAll', () => {
    it('should return an array of transaction summaries', async () => {
      ...
      const result = await service.findAll(1, UserRole.USER, {
    ...
    it('should NOT return transactions where user is creator and no forwards (Creator Inbox) in Default view', async () => {
      ...
      const resultInbox = await service.findAll(1, UserRole.USER, {
      ...
      prismaMock.$queryRaw.mockResolvedValue([]);
      prismaMock.transaction.findMany.mockResolvedValue([]);
      const resultOutgoing = await service.findAll(1, UserRole.USER, {
    ...
    it('should return transactions where user is receiver of latest forward (Inbox)', async () => {
      ...
      prismaMock.$queryRaw.mockResolvedValue([]);
      prismaMock.transaction.findMany.mockResolvedValue([]);
      const resultDefault = await service.findAll(1, UserRole.USER, {});
  ...
  describe('findOne', () => {
    ...
    it('should return a transaction if found', async () => {
      ...
      const result = await service.findOne(id, UserRole.USER);
  ...
  describe('remove', () => {
    ...
    it('should successfully remove a transaction', async () => {
      ...
      await service.remove(id, UserRole.USER);
  ...
  describe('detachDocument', () => {
    ...
    it('should detach a document using deleteMany', async () => {
      ...
      await service.detachDocument(transactionId, documentId, UserRole.USER);
